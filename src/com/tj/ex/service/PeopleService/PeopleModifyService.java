...
public class PeopleModifyService implements Service {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		String path = request.getRealPath("FileBoardUp/2_PeopleUp");
		...
		try {
			...
			String dbFileName = mRequest.getParameter("pFilename");
			if (pFilename == null) {
				pFilename = dbFileName;
			}
			int pNum = Integer.parseInt(mRequest.getParameter("pNum"));
			String pTitle = mRequest.getParameter("pTitle");
			String pContent = mRequest.getParameter("pContent");
			String pPw = mRequest.getParameter("pPw");
			PeopleDao Dao = new PeopleDao();
			int result = Dao.modify(pNum, pTitle, pContent, pFilename, pPw);
		...
		if (serverFile.exists()) {
			...
			try {
				...
				os = new FileOutputStream("D:/mega_IT/source/6_JSP/MyProject/WebContent/FileBoardUp/2_PeopleUp/" + pFilename);
